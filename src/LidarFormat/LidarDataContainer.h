...
public:
    ...
    /// WARNING: erases data but keeps attibute maps in memory
    void clear();
    ...
    /// Display attribute list on a line
    ...
    /// Add an attribute with its type to the container
    /// WARNING : calling this cette fonction makes all current itertors obsolete, and makes former LidarEchos incompatible with new ones
    ///  returns false is the attribute was already in the container
    ...
    /// WARNING: returns a pointer to the data in the container -> dangerous !
    /// (used by classe LidarFile to load binary data in a single operation
